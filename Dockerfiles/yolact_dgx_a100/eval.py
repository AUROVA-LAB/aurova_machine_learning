...
def parse_args(argv=None):
    ...
    parser.add_argument('--map_test', default=False, type=str, help="If you want to get mAP of test dataset, add --map_test")

    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        benchmark=False, no_sort=False, no_hash=False, mask_proto_debug=False, crop=True, detect=False, display_fps=False,
                        emulate_playback=False, map_test=False)
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    if not args.output_coco_json:
        with timer.env('Prepare gt'):
            ...
            for i in range(len(gt)):
                new_gt.append(gt[i][:4])
                class_gt.append(gt[i][4])

            #print(new_gt)
            #print(class_gt)
            gt_boxes = torch.Tensor(new_gt)

            #gt_boxes = torch.Tensor(gt[:, :4])
            gt_boxes[:, [0, 2]] *= w
            gt_boxes[:, [1, 3]] *= h
            gt_classes = list(class_gt)
    ...
    #print("l. 469: "+str(set(classes + gt_classes)))
    for _class in set(classes + gt_classes):
        #print("l471: "+str(_class))
        ap_per_iou = []
        num_gt_for_class = sum([1 for x in gt_classes if x == _class])
        #print("l474: "+str(num_gt_for_class))
        
        #print("l476: "+str(iou_thresholds))
        #print("l477: "+str(len(iou_thresholds)))
        for iouIdx in range(len(iou_thresholds)):
            iou_threshold = iou_thresholds[iouIdx]
            #print("l480: "+str(iou_threshold))
            for iou_type, iou_func, crowd_func, score_func, indices in iou_types:
                #print("l482: "+str(iou_type)+", "+str(iou_func)+", "+str(crowd_func)+", "+str(score_func)+", "+str(indices))
                #print("l483: "+str(len(gt_classes)))
                gt_used = [False] * len(gt_classes)
                #print("l485: "+str(ap_data)+", iouIdx: "+str(iouIdx)+", class: "+str(_class))
                ap_obj = ap_data[iou_type][iouIdx][_class]
                #print("l487: "+str(ap_obj))
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            with timer.env('Network Extra'):
                ...
                start_time = time.time()
                #print("ART: "+str(start_time))

                preds = net(batch)

                stop_time = time.time()
                #print("TOP: "+str(stop_time))
                one_time = stop_time - start_time
                print("IME: "+str(one_time))

                total_time = total_time + one_time
                #print("T_IME: "+str(total_time))
        ...
        if not args.display and not args.benchmark:
            ...
            if args.output_coco_json:
                ...
                if not train_mode:
                    ...
                    total_time = total_time / dataset_size
                    #print(dataset_size)
                    print(str(total_time*1000)+" msec.")
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if args.image is None and args.video is None and args.images is None:
            if args.map_test == False:
                dataset = COCODetection(cfg.dataset.valid_images, cfg.dataset.valid_info,
                                        transform=BaseTransform(), has_gt=cfg.dataset.has_gt)
            else:
                print("Test_map...")
                dataset2 = COCODetection(cfg.dataset.test_images, cfg.dataset.test_info,
                                         transform=BaseTransform(), has_gt=cfg.dataset.has_gt)
            prep_coco_cats()
        else:
            dataset = None        
